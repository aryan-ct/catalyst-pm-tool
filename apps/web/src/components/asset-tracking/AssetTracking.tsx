...
import { Eye, EyeOff, Search, Trash2, ChevronRight } from 'lucide-react';
...
export default function AssetTracking() {
  ...
  const fetchAssets = async () => {
    try {
      setAssets(await ASSET_API.getAll());
    } catch {
      /* ignore */
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this asset record? This cannot be undone.'))
      return;
    setDeletingId(id);
    try {
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((asset) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_BADGE[asset.status]}`}
                    >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        onClick={() => navigate(`/asset-tracking/${asset.id}`)}
