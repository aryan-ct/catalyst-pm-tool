...
const STATUS_TABS: { label: string; value: AssetStatus | 'ALL' }[] = [
  { label: 'All', value: 'ALL' },
...
export default function AssetTracking() {
  ...
  const fetchAssets = async () => {
    try {
      const data = await ASSET_API.getAll();
      setAssets(data);
    } catch (err) {
  ...
  const tabCounts = STATUS_TABS.reduce<Record<string, number>>((acc, tab) => {
    acc[tab.value] =
      tab.value === 'ALL' ? assets.length : assets.filter((a) => a.status === tab.value).length;
    return acc;
  }, {});
    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-border overflow-x-auto">
        {STATUS_TABS.map((tab) => (
          <button
            ...
            className={`px-5 py-2.5 text-sm font-medium whitespace-nowrap capitalize transition-colors border-b-2 -mb-px ${
              activeTab === tab.value
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="relative w-full sm:w-[280px]">
          ...
          <Input
            placeholder="Search by name, product, serial…"
            value={search}
      ...
      <div className="rounded-lg border border-border overflow-x-auto bg-white">
        <table className="w-full text-sm">
          <thead className="bg-muted/40 border-b border-border">
            <tr>
              {[
                'Record Name', 'Product', 'Product Name', 'Serial No.',
                'Status', 'Condition', 'Allocated To',
                'Date of Allocation', 'Asset Price', 'PIN/Password', 'Actions',
              ].map((h) => (
      ...
      {/* Edit modal — rendered outside the table to avoid nesting issues */}
      {editAsset && (
        <AssetModal
          editData={editAsset}
