...
import { ASSET_API, AssetRepair, RepairStatus } from '@/api/asset-tracking.api';
import { Loader2, Plus, ArrowLeft } from 'lucide-react';

interface AssetRepairModalProps {
  assetId: string;
  assetName: string;
  onClose: () => void;
  onRefresh?: () => void;
...
export default function AssetRepairModal({ assetId, assetName, onClose, onRefresh }: AssetRepairModalProps) {
  ...
  const fetchRepairs = async () => {
    setLoading(true);
    try {
      const data = await ASSET_API.getRepairs(assetId);
      setRepairs(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingRepair) {
        ...
        await ASSET_API.createRepair(assetId, payload);
      }
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[85vh] min-w-[700px] overflow-y-auto p-6">
